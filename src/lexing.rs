...
impl<C: Iterator<Item = Result<DecodedChar, E>>, E> Lexer<C, E> {
	...
	pub fn consume(&mut self) -> Result<(Option<Token>, Span), (Error<E>, Span)> {
		...
		match self.next_char()? {
			Some('@') => Ok({
				let t = self.next_langtag_or_keyword()?;
				let token = match t.0 {
			...
			Some('<') => Ok({
				let t = self.next_iriref()?;
				(Some(Token::IriRef(t.0)), t.1)
			}),
			Some('"') => Ok({
				let t = self.next_string_literal('"')?;

				(Some(Token::StringLiteral(t.0)), t.1)
			}),
			Some('\'') => Ok({
				let t = self.next_string_literal('\'')?;
			...
			Some('_') => Ok({
				let t = self.next_blank_node_label()?;

				(Some(Token::BlankNodeLabel(t.0)), t.1)
			}),
			...
			Some(c @ ('+' | '-' | '0'..='9' | '.')) => Ok({
				let t = self.next_numeric_or_dot(c)?;

				let token = match t.0 {
					NumericOrPeriod::Numeric(n) => Token::Numeric(n),
					NumericOrPeriod::Period => Token::Punct(Punct::Period),
				};

				(Some(token), t.1)
			}),
			Some(c) => Ok({
				let t = self.next_name_or_keyword(c)?;

				let token = match t.0 {
					NameOrKeyword::Keyword(kw) => Token::Keyword(kw),
					NameOrKeyword::CompactIri(p, s) => Token::CompactIri(p, s),
				};
